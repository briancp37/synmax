        ...
        for edge in plan.edges:
            ...
            if edge.dst not in step_ids:
                raise DSLValidationError(
                    f"Edge references non-existent destination step: {edge.dst}"
                )
        ...
        if step.op == StepType.FILTER:
            ...
            for param in required_params:
                if param not in step.params:
                    raise DSLValidationError(
                        f"Filter step {step.id} missing required parameter: {param}"
                    )
            ...
            if "metrics" not in step.params:
                raise DSLValidationError(
                    f"Aggregate step {step.id} missing required parameter: metrics"
                )

            metrics = step.params["metrics"]
            if not isinstance(metrics, list) or not metrics:
                raise DSLValidationError(
                    f"Aggregate step {step.id} metrics must be a non-empty list"
                )
            ...
            for param in required_params:
                if param not in step.params:
                    raise DSLValidationError(
                        f"Changepoint step {step.id} missing required parameter: {param}"
                    )
            ...
            if not isinstance(step.params["n"], int) or step.params["n"] <= 0:
                raise DSLValidationError(
                    f"Limit step {step.id} 'n' parameter must be a positive integer"
                )
