...
from .agent_schema import Edge, PlanGraph, Step


def _tiny_sum_by_group(
    group: list[str], date_range: tuple[str, str] | None, top: int = 10
) -> PlanGraph:
    ...
    nodes = [
        (
            Step(
                id="f",
                op="filter",
                params={"column": "eff_gas_day", "op": "between", "value": list(date_range)},
            )
            if date_range
            else None
        ),
        Step(
            id="a",
            op="aggregate",
            params={"groupby": group, "metrics": [{"col": "scheduled_quantity", "fn": "sum"}]},
        ),
    ...
    outputs = (
        [nodes[-2].id]
        if len(nodes) > 1 and nodes[-1].op == "evidence_collect"
        else [nodes[-1].id] if nodes else []
    )
...
def _tiny_count_by_group(
    ...
    nodes = [
        ...
        Step(
            id="a",
            op="aggregate",
            params={"groupby": group, "metrics": [{"col": "scheduled_quantity", "fn": "count"}]},
        ),
...
def build_tiny_dag_from_legacy_args(q: str, opts: dict[str, Any]) -> PlanGraph:
    ...
    if any(
        phrase in low
        for phrase in ["daily total", "daily sum", "daily volume", "per day", "by day"]
    ):
