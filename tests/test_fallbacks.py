...
from data_agent.core.agent_schema import PlanGraph, Step, Edge, StepType
...
class TestDSLLoader:
    ...
    def test_validate_aggregate_params(self):
        ...
        plan_data = {
            "nodes": [
                {"id": "a", "op": "aggregate", "params": {"groupby": ["pipeline_name"]}}  # Missing metrics
            ],
            "edges": [{"src": "raw", "dst": "a"}],
            "inputs": ["raw"],
            "outputs": ["a"]
...
class TestMacros:
    ...
    def test_create_time_series_analysis_macro(self):
        ...
        assert isinstance(plan, PlanGraph)
        assert len(plan.nodes) >= 6  # Should have filter, aggregate, stl, changepoint, rank, limit, evidence
...
class TestAgentPlannerFallbacks:
    ...
    def test_fallback_aggregation_pattern(self):
        ...
        plan = planner.plan(
            "sum the quantities by pipeline",
            available_columns,
            fallback=True
        )
