...
class TestAgentIntegration:
    ...
    def test_golden_plan_regime_shifts_2021(self, examples_dir: Path, golden_dataset: pl.DataFrame):
        ...
        if expected_path.exists():
            ...
            if expected_data and result_data:
                ...
                assert (
                    not missing_core
                ), f"Missing core columns: {missing_core}. Available: {result_cols}"
        ...
        assert (
            "plan_hash" in plan_evidence or "hash" in plan_evidence
        ), f"Missing hash field in plan evidence: {plan_evidence.keys()}"
    ...
    def test_synthetic_stl_detection(self):
        ...
        assert (
            "deseasonalized" in result_cols
        ), f"Expected deseasonalized column not found. Available: {result_cols}"
    ...
    def test_synthetic_changepoint_detection(self):
        ...
        for col in expected_cp_cols:
            assert any(
                col in result_col for result_col in result_cols
            ), f"Expected changepoint column {col} not found"
