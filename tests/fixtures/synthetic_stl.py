...
def generate_seasonal_series(
    ...
    for i, pipeline_name in enumerate(pipeline_names):
        ...
        annual_seasonal = annual_amplitude * np.cos(
            2 * np.pi * (day_of_year - 1) / 365.25 + pipeline_phase
        )
...
def validate_stl_components(
    ...
    weekly_ok = (
        abs(weekly_amplitude - expected_weekly_amplitude) / expected_weekly_amplitude <= tolerance
    )
    ...
    annual_ok = (
        abs(annual_amplitude - expected_annual_amplitude) / expected_annual_amplitude <= tolerance
    )
...
if __name__ == "__main__":
    ...
    print(
        df_shift.group_by("regime_shift_flag").agg(
            [pl.col("value").mean().alias("mean_value"), pl.col("value").count().alias("count")]
        )
    )
