...
def generate_changepoint_series(
    ...
    if len(regime_values) != len(changepoints) + 1:
        raise ValueError(
            f"regime_values length ({len(regime_values)}) must be len(changepoints) + 1 ({len(changepoints) + 1})"
        )
...
def generate_multiple_changepoint_series(
    start_date: str, end_date: str, pipeline_configs: List[dict], seed: int = 42
) -> pl.DataFrame:
    """
...
def validate_changepoint_detection(
    detected_changepoints: List[str], true_changepoints: List[str], tolerance_days: int = 7
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    print(
        df_gradual.group_by(pl.col("transition_progress").round(1))
        .agg([pl.col("value").mean().alias("mean_value"), pl.col("value").count().alias("count")])
